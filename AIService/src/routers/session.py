...
async def _run_full_pipeline(
    ...
    try:
        ...
        if save_to_db and django_session_id:
            ...
            ready_items = ["emotional_profile", "ai_insights"]
            try:
                ...
                ready_items.insert(0, "soap")
                logger.info("[PIPELINE] Step 11 — SOAP draft generated")
            except Exception as soap_exc:
                logger.warning(f"[PIPELINE] Step 11 — SOAP draft generation failed: {soap_exc}")

            await _notify_backend_ai_outputs_ready(django_session_id, ready_items=ready_items)
            logger.info("[PIPELINE] Step 12 — backend notified for therapist notification")
...
async def _notify_backend_ai_outputs_ready(session_id: str, ready_items: Optional[list[str]] = None) -> None:
    ...
    payload = {
        "session_id": session_id,
        "ready_items": ready_items or ["emotional_profile", "ai_insights"],
    }
    headers = {
