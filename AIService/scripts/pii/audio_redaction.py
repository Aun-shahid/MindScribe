...
def _build_presidio_analyzer(spacy_model_name: str):
    ...
    try:
        ...
        # Add custom Credit Card recognizer to bypass Luhn check for testing
        cc_pattern = Pattern(
            name="cc_pattern",
        ...
        # Add custom Address recognizer (handles "Makan number", "Gali", "Phase", etc.)
        # This is a generic pattern, not hardcoded to specific numbers.
        address_pattern = Pattern(
            name="address_pattern",
            regex=r"\b(?:makan|gali|house|street|sector|phase|plot|apartment|flat|home|building|block|unit)\s*(?:number|no|#)?\s*\d+[a-z]?\b",
            score=0.85
        )
        address_recognizer = PatternRecognizer(
            supported_entity="LOCATION",
            patterns=[address_pattern],
            context=["makan", "gali", "house", "street", "address", "location", "resident"]
        )
