...
import {
  ...
  Modal, useWindowDimensions, StatusBar, ScrollView,
} from 'react-native';
...
// Keep this so in-app foreground notifications display correctly
              ...
              return (
                <Animated.View key={opt.value} style={{ transform: [{ scale: scaleAnims[i] }] }}>
                  ...
                  >
                    {active && (
                      <LinearGradient
                        colors={['rgba(167,139,250,0.28)', 'rgba(255,179,107,0.16)']}
...
export default function NotificationSettings() {
  ...
  const load = async () => {
    try {
      setLoading(true);
      // GET /patients/notifications/preferences/
  ...
  // Cancel any previously scheduled local reminders on mount
  // so old frontend-scheduled ones don't fire anymore
  useEffect(() => {
    const cancelAllLocal = async () => {
      try {
  ...
  const save = async () => {
    ...
    try {
      setSaving(true);
      // PUT /patients/notifications/preferences/
      // Backend scheduler reads these preferences and sends notifications accordingly
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#342949' }}>
      ...
      <LinearGradient colors={['#342949', '#2A1F3D', '#342949']}
        style={[StyleSheet.absoluteFill, { height }]} pointerEvents="none" />
      ...
      <StickyHeader scrollY={scrollY} firstWord="Notification" secondWord="Settings" onBackPress={() => router.back()} />
      ...
      >
        ...
        <GlassCard accent="#FFD93D">
          ...
          <ToggleRow
            ...
            hint="Backend sends a reminder if you haven't logged mood by your set time"
        ...
        <GlassCard accent="#5DADE2">
          ...
          <ToggleRow
            label="Daily Reminder"
            hint="Backend sends a reminder if you haven't written a journal entry by your set time"
        ...
        <GlassCard accent="#A78BFA">
          ...
          <View style={{ marginTop: 10, borderTopWidth: 1, borderTopColor: 'rgba(255,255,255,0.06)', paddingTop: 10 }}>
            ...
            <ToggleRow
              ...
              hint="When therapist confirms your appointment"
            ...
            <ToggleRow
              ...
              hint="When a session is cancelled or rescheduled"
        ...
        <GlassCard accent="#FFB36B">
          <CardHeader emoji="🔔" title="Other Reminders" subtitle="Goals & therapist messages" />
          <ToggleRow
            label="Goal Reminders"
            hint="Reminders to check in on your therapy goals"
          ...
          <ToggleRow
            ...
            hint="Notifications when your therapist sends a message"
            value={!!prefs.therapist_messages_enabled}
            onChange={v => setPrefs({ ...prefs, therapist_messages_enabled: v })}
            accent="#FFB36B"
            isLast
          />
        </GlassCard>
          ...
          <TouchableOpacity onPress={save} disabled={saving} activeOpacity={0.85}>
            ...
            >
              {saving
                ...
                : <><FontAwesome name="check-circle" size={16} color="#FFFFFF" style={{ marginRight:9 }} /><Text style={sv.text}>Save Preferences</Text></>
              }
            </LinearGradient>
          </TouchableOpacity>
        </Animated.View>
