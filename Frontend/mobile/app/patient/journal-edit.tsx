...
// card gradient — matches CreateJournal
...
export default function JournalEdit() {
  ...
  // ── Responsive tokens ──────────────────────────────────────────────────────
  const pi           = clamp(width * 0.048,  16, 24);   // page inset
  const si           = clamp(width * 0.04,   14, 20);   // section inset (scroll)
  ...
  // card / layout
  const cR        = clamp(width * 0.042,  13, 16);   // card radius
  const cPad      = clamp(width * 0.045,  14, 18);   // card padding
  const sGap      = clamp(height * 0.028, 18, 24);   // section gap
  const cBot      = clamp(height * 0.05,  30, 46);   // bottom pad
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      const fly = (
        y: Animated.Value, x: Animated.Value,
        dY: number, dX: number
      ) => {
          ...
          <Animated.View style={{ opacity: fadeAnim }}>
            ...
            <View style={[s.card, {
              ...
              <LinearGradient
                ...
                style={[StyleSheet.absoluteFill, { borderRadius: cR, zIndex: -1 }]}
                pointerEvents="none"
              />
              {/* Accent strip */}
              ...
              <View style={{ padding: cPad }}>
                ...
                <View style={[s.cardHeaderRow, { marginBottom: clamp(height * 0.018, 12, 16) }]}>
                  ...
                  <View>
                    <Text style={{ color: '#FFFFFF', fontSize: cardTitleSz, fontWeight: '800', letterSpacing: 0.5 }}>
                      Entry Title
                    </Text>
                    <Text style={{ color: '#9D8EC7', fontSize: clamp(width * 0.029, 10, 11), letterSpacing: 1.2, marginTop: 1 }}>
                      REQUIRED
                    </Text>
                  </View>
                </View>
            ...
            <View style={[s.card, {
              ...
              <View style={{ padding: cPad }}>
                ...
                <View style={[s.cardHeaderRow, { marginBottom: clamp(height * 0.014, 10, 14) }]}>
                  ...
                  <View style={{ flex: 1 }}>
                    <Text style={{ color: '#FFFFFF', fontSize: cardTitleSz, fontWeight: '800', letterSpacing: 0.4 }}>
                      What's on your mind?
                    </Text>
                ...
                <View style={{
                  ...
                  <Text style={{ color: '#9D8EC7', fontSize: wordCntSz, fontStyle: 'italic' }}>
                    {wordCount} words
                  </Text>
                </View>
              </View>
            ...
            <View style={[s.card, {
              ...
              <View style={{ marginTop: clamp(height * 0.008, 4, 8) }}>
                <Text style={{ color: '#FFFFFF', fontSize: cardTitleSz, fontWeight: '700', marginBottom: clamp(height * 0.008, 5, 7) }}>
                  🎨 Tags
                  <Text style={{ color: '#9D8EC7', fontWeight: '500' }}> (Optional)</Text>
                </Text>
                    ...
                    return (
                      <TouchableOpacity
                        ...
                        style={{
                          ...
                          backgroundColor:   isSelected ? tagColor : tagLightBg,
                        }}
                      >
                        <Text style={{ color: '#FFFFFF', fontSize: pillTxtSz, fontWeight: '600' }}>
                          {tag}
                        </Text>
                      </TouchableOpacity>
                    );
            ...
            <View style={[s.card, {
              ...
              >
                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                  <View style={{
                    width:  clamp(width * 0.086, 30, 36),
                    height: clamp(width * 0.086, 30, 36),
                  ...
                  <View>
                    <Text style={{ color: '#FFFFFF', fontSize: switchLblSz, fontWeight: '700' }}>
                      Mark as Favourite
                    </Text>
                    <Text style={{ color: '#CEC2EE', fontSize: switchSubSz, marginTop: 2 }}>
                ...
                {/* Toggle switch */}
                <View style={{
                  width: switchW, height: switchH, borderRadius: switchR, padding: 2,
                  borderWidth: 1,
                  borderColor: formData.is_favorite ? '#FFD27A' : 'rgba(255,255,255,0.24)',
                  backgroundColor: formData.is_favorite ? '#F5A623' : '#8E87A8',
                }}>
                  <View style={{
                    width: switchThumbSz, height: switchThumbSz,
                    borderRadius: switchThumbSz / 2,
                    backgroundColor: '#FFFFFF',
                    transform: [{
                      translateX: formData.is_favorite
                        ? clamp(width * 0.055, 20, 24)
                        : 2,
                    }],
            ...
            <View style={[s.card, {
              ...
              >
                ...
                <View style={{
                  width: switchW, height: switchH, borderRadius: switchR, padding: 2,
                  borderWidth: 1,
                  borderColor: formData.is_private ? 'rgba(167,139,250,0.6)' : 'rgba(255,255,255,0.24)',
                  backgroundColor: formData.is_private ? '#7C5CE0' : '#8E87A8',
                }}>
                  <View style={{
                    width: switchThumbSz, height: switchThumbSz,
                    borderRadius: switchThumbSz / 2,
                    backgroundColor: '#FFFFFF',
                    transform: [{
                      translateX: formData.is_private
                        ? clamp(width * 0.055, 20, 24)
                        : 2,
                    }],
                    shadowColor: '#000', shadowOpacity: 0.2,
            ...
            >
              <Text style={{ color: '#CEC2EE', fontSize: clamp(width * 0.04, 14, 16), fontWeight: '600' }}>
                Cancel
              </Text>
            </TouchableOpacity>
