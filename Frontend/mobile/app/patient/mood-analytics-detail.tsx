...
export default function MoodAnalyticsDetail() {
  ...
  // Reload whenever selectedDays changes or screen focuses
  useFocusEffect(
    useCallback(() => {
      loadAnalytics();
      return () => {};
  ...
  // ── Navigation — always back to mood screen ───────────────────────────────
  const goBack = () => router.push('/patient/mood');
  ...
  if (!analytics) {
    return (
      <View style={[styles.container, { backgroundColor: screenBackground }]}>
        <StatusBar barStyle="light-content" backgroundColor={screenBackground} />
        <LinearGradient
          colors={[screenBackground, '#2A1F3D', screenBackground]}
          style={styles.bgGradient}
          pointerEvents="none"
        />
        <View style={[styles.emptyContainer, { padding: emptyPad }]}>
          <Text style={{ fontSize: emptyEmojiSz, marginBottom: emptyEmojiGap }}>📊</Text>
          <Text style={{ color: themeStyle.text, fontSize: emptyTxtSz, fontWeight: '600', marginBottom: emptyTxtGap }}>
            No mood data available
          </Text>
          <Text style={{ color: themeStyle.label, fontSize: emptySubSz, textAlign: 'center', marginBottom: emptySubGap }}>
            Start tracking your moods to see analytics
          </Text>
          <TouchableOpacity
            style={{
              backgroundColor: themeStyle.button,
              borderRadius: actBtnR,
              paddingHorizontal: actBtnPadH,
              paddingVertical: actBtnPadV,
            }}
            onPress={goBack}
          >
            <Text style={{ color: themeStyle.buttonText, fontSize: actBtnTxtSz, fontWeight: '600' }}>
              Go Back
            </Text>
          </TouchableOpacity>
        </View>
  ...
  return (
    <View style={[styles.container, { backgroundColor: screenBackground }]}>
      ...
      {/* Sticky header — routes to mood screen */}
      <StickyHeader
        scrollY={scrollY}
        firstWord="Mood"
        ...
        style={[
          styles.headerContainer,
          {
            ...
            opacity: scrollY.interpolate({
              inputRange: [0, hFadeDist * 0.45, hFadeDist],
              outputRange: [1, 0, 0],
              extrapolate: 'clamp',
            }),
            transform: [{
              translateY: scrollY.interpolate({
                inputRange: [0, hFadeDist],
                outputRange: [0, -10],
                extrapolate: 'clamp',
              }),
            }],
      ...
      >
        {/* Back button — goes to /patient/mood */}
        <TouchableOpacity
      ...
      >
        ...
        <View style={[styles.timePeriodRow, { marginBottom: cGap }]}>
          <TouchableOpacity
            style={[styles.timePeriodTrigger, {
              paddingVertical: filtPadV,
              paddingHorizontal: filtPadH,
              borderRadius: filtR,
            }]}
            activeOpacity={0.85}
          ...
          >
            ...
            <Text style={[styles.timePeriodTriggerText, { fontSize: filtTxtSz }]}>
        ...
        <View style={{ borderRadius: cR, overflow: 'hidden', marginBottom: cGap }}>
          <LinearGradient
            colors={[topMoodColors[0] + '33', topMoodColors[1] + '22', '#342949']}
          ...
          >
            <View style={[styles.heroCard, { padding: cPad, borderRadius: cR }]}>
              ...
              <View style={[styles.heroTopRow, isCompactLayout && styles.heroTopRowCompact]}>
                <LinearGradient
                  colors={topMoodColors}
                  style={[styles.heroBadge, {
                    width:        isCompactLayout ? domCircleSz * 0.86 : domCircleSz,
                    height:       isCompactLayout ? domCircleSz * 0.86 : domCircleSz,
                    borderRadius: isCompactLayout ? (domCircleSz * 0.86) / 2 : domCircleSz / 2,
                    marginBottom: isCompactLayout ? clamp(height * 0.012, 8, 12) : 0,
                  }]}
                >
                ...
                <View style={[styles.heroTextBlock, isCompactLayout && styles.heroTextBlockCompact]}>
                  ...
                  <Text style={[styles.heroMoodName, { fontSize: clamp(width * 0.064, 22, 28) }]}>
                    {getMoodLabel(topMood)}
                  </Text>
                  <Text style={[styles.heroSubLabel, { fontSize: clamp(width * 0.033, 12, 14) }]}>
                    {analytics.mood_distribution[topMood] ?? 0}× recorded this period
                  </Text>
                </View>
              </View>
              ...
              <View style={{ marginTop: clamp(height * 0.022, 14, 20) }}>
                <View style={styles.intensityLabelRow}>
                  <Text style={[styles.intensityLabel, { fontSize: clamp(width * 0.031, 11, 13) }]}>
                    AVERAGE INTENSITY
                  </Text>
                ...
                <View style={[styles.intensityTrack, {
                  borderRadius: clamp(height * 0.006, 4, 6),
                  height: clamp(height * 0.012, 8, 11),
                }]}>
                  <LinearGradient
                    colors={topMoodColors}
                    start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}
                    style={[styles.intensityFill, {
                      width: `${intensityPct}%`,
                      borderRadius: clamp(height * 0.006, 4, 6),
                    }]}
                  />
              ...
              <View style={[styles.heroStatsRow, { marginTop: clamp(height * 0.022, 14, 20), gap: metricsGap }]}>
                <View style={styles.heroStat}>
                  ...
                  <Text style={[styles.heroStatLabel, { fontSize: clamp(width * 0.029, 10, 12) }]}>
                    {'TOTAL\nENTRIES'}
                  </Text>
                </View>
                <View style={styles.heroStatDivider} />
                <View style={styles.heroStat}>
                  <Text style={[styles.heroStatValue, { fontSize: metricValSz }]}>
        ...
        {sortedDist.length > 0 && (
          <View style={[styles.card, {
            backgroundColor: '#2C2344', borderRadius: cR,
            padding: cPad, marginBottom: cGap,
          }]}>
            <View style={styles.sectionHeaderRow}>
              <Text style={[styles.sectionTitle, { color: '#FFFFFF', fontSize: secTitleSz }]}>
                Mood Breakdown
              </Text>
              <View style={[styles.sectionBadge, { borderRadius: clamp(width * 0.03, 10, 14) }]}>
                <Text style={[styles.sectionBadgeText, { fontSize: clamp(width * 0.028, 10, 12) }]}>
                  {sortedDist.length} moods
                </Text>
              </View>
            </View>
                  ...
                  <View key={mood}>
                    <View style={[styles.distributionHeader, { marginBottom: distItemGap }]}>
                      <View style={styles.distributionMoodInfo}>
                        <View style={[styles.distEmojiCircle, {
                          width:        clamp(width * 0.094, 34, 40),
                          height:       clamp(width * 0.094, 34, 40),
                          borderRadius: clamp(width * 0.047, 17, 20),
                          backgroundColor: colors[0] + '28',
                          borderColor:     colors[0] + '55',
                        }]}>
                          <Text style={{ fontSize: distEmojiSz }}>{getMoodEmoji(mood)}</Text>
                        </View>
                        <View>
                          <Text style={[styles.distributionLabel, { color: '#FFFFFF', fontSize: bodyTxtSz }]}>
                            {getMoodLabel(mood)}
                          </Text>
                          {isTop && (
                            <Text style={[styles.topBadgeText, { fontSize: clamp(width * 0.026, 9, 11) }]}>
                              TOP MOOD
                            </Text>
                          )}
                        </View>
                      ...
                      <View style={styles.distCountBlock}>
                        <Text style={[styles.distributionCount, { color: '#FFFFFF', fontSize: clamp(width * 0.04, 14, 16) }]}>
                          {pct.toFixed(0)}%
                        </Text>
                        <Text style={[styles.distCountSub, { fontSize: clamp(width * 0.028, 10, 12) }]}>
                          {count}×
                        </Text>
                      </View>
                    </View>
                    <View style={[styles.distributionBarBackground, {
                      backgroundColor: '#3E3258',
                      height: distBarH, borderRadius: distBarR,
                    }]}>
                      <LinearGradient
                        colors={colors}
                        start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}
                        style={[styles.distributionBar, { width: `${pct}%`, borderRadius: distBarR }]}
                      />
                    </View>
                  </View>
                );
        ...
        {trendPoints.length > 0 && (() => {
          ...
          const barW      = Math.max(
            (innerW - sparkGap * Math.max(trendPoints.length - 1, 0)) / trendPoints.length,
            clamp(width * 0.028, 12, 20)
          );
          ...
          return (
            <View style={[styles.card, {
              ...
              <Text style={[styles.sectionTitle, { color: '#FFFFFF', fontSize: secTitleSz, marginBottom: secTitleBotGap }]}>
                Weekly Trend
              </Text>
                ...
                {trendPoints.map((point, i) => {
                  ...
                  return (
                    <View key={i} style={[styles.sparkBarWrap, { width: barW }]}>
                      <LinearGradient
                        colors={['#A78BFA', '#6D5DD3']}
                        start={{ x: 0, y: 0 }} end={{ x: 0, y: 1 }}
                        style={[styles.sparkBar, { height: barH, borderRadius: clamp(barW * 0.3, 3, 6) }]}
                      />
                      <Text style={[styles.sparkLabel, { fontSize: clamp(width * 0.022, 8, 10) }]} numberOfLines={1}>
                        {dateStr}
                      </Text>
                    </View>
                  );
              ...
              {analytics.common_triggers.map((trigger, index) => (
                <LinearGradient
                  ...
                  style={[styles.triggerChip, {
                    borderRadius: triggerChipR,
                    paddingHorizontal: clamp(width * 0.04, 14, 18),
                    paddingVertical: clamp(height * 0.012, 8, 10),
                    borderColor: 'rgba(167,139,250,0.35)',
                  }]}
                >
                  <Text style={[styles.triggerText, { color: '#E8DFFF', fontSize: triggerTxtSz }]}>
                    {trigger}
                  </Text>
                </LinearGradient>
              ))}
        ...
        <LinearGradient
          ...
          style={[styles.infoCard, {
            borderRadius: cR,
            marginBottom: cGap,
            padding: clamp(width * 0.045, 16, 20),
        ...
        >
          ...
          <Text style={[styles.infoText, {
            color: '#CFC7EF', fontSize: bodyTxtSz,
            lineHeight: clamp(width * 0.05, 18, 22),
          }]}>
        ...
        >
          <View style={[styles.modalContent, { backgroundColor: '#473F5A' }]}>
            <View style={styles.modalHeaderRow}>
              <Text style={[styles.modalTitle, { color: '#FFFFFF', fontWeight: '700', fontSize: modalTitleSz }]}>
                Time Period
              </Text>
              <TouchableOpacity
            ...
            {dayOptions.map((days) => (
              <TouchableOpacity
                ...
                style={[styles.modalOption, {
                  paddingVertical:   modalOptPadY,
                  paddingHorizontal: modalOptPadX,
                  borderRadius:      modalOptR,
                  backgroundColor:   selectedDays === days ? '#5B5270' : 'transparent',
                }]}
                onPress={() => { setSelectedDays(days); setShowPeriodModal(false); }}
              >
                <Text style={[styles.modalOptionText, { color: '#FFFFFF', fontSize: modalOptTxtSz }]}>
                  Last {days} days
                </Text>
                {selectedDays === days && (
                  <FontAwesome name="check" size={14} color="#FFB36B" />
                )}
              </TouchableOpacity>
            ))}
          </View>
